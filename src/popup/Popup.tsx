...
import { loadSettings, saveSettings, addItemToSetting, removeItemFromSetting } from '../utils/storage';
import './Popup.css';
...
const Popup: React.FC = () => {
  ...
  const updateResults = () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (!tabs[0] || !tabs[0].url || !tabs[0].url.includes('moneyforward.com/cf')) {
        showStatusMessage('MoneyForwardの家計簿ページを開いてください', 'error');
        return;
      }
      ...
      try {
        chrome.tabs.sendMessage(tabs[0].id!, { action: 'getResults' }, (response) => {
          ...
          if (chrome.runtime.lastError) {
            ...
            showStatusMessage('コンテンツスクリプトとの通信エラー。ページを再読み込みしてください', 'error');
          ...
          if (response && response.results) {
            ...
            showStatusMessage('MoneyForwardの家計簿ページでデータを取得できませんでした', 'error');
          }
        });
      } catch (error) {
  ...
  const showStatusMessage = (text: string, type: 'info' | 'success' | 'error') => {
  ...
  return (
    <div className="container">
      ...
      <div className={`tab-content ${activeTab === 'results' ? 'active' : ''}`}>
        ...
        <button className="button" onClick={refreshData}>データを更新</button>
      </div>
      ...
      <div className={`tab-content ${activeTab === 'settings' ? 'active' : ''}`}>
        ...
        <div className="input-group">
          ...
          <div style={{ display: 'flex', marginTop: '8px' }}>
            ...
            <button 
              className="button" 
              style={{ marginLeft: '8px' }}
              onClick={() => handleAddItem('expenceSubList', 'expenseSubcategory')}
            >
              追加
            </button>
          </div>
        ...
        <button className="button" onClick={handleSaveSettings}>設定を保存</button>
      </div>
    </div>
  );
};
